...
def check_if_exists_by_file_name(file_name):
    if USE_INT_FILENAMES:
        ...
        with DB_filename_to_id.begin(buffers=True) as txn:
            file_id = txn.get(f"{IMAGE_PATH}/{file_name}".encode(), default=False)
            if not file_id:
                return False
...
def fast_phash(image, hash_size=24, highfreq_factor=4):  # hash_size=16 for 256bit #hash_size=8 for 64bit
    ...
    image = image.resize((img_size, img_size),
                         Image.Resampling.LANCZOS)  # cv2.resize(image, (img_size, img_size), interpolation=cv2.INTER_AREA)  # cv2.INTER_AREA
...
for batch in new_images:
    ...
    for i in range(len(phashes)):
        if USE_INT_FILENAMES:
            ...
            file_name = phashes[i][0].encode()
            seq_global_id_bytes = int_to_bytes(SEQUENTIAL_GLOBAL_ID)
            phashes[i][0] = seq_global_id_bytes
            phashes[i] = tuple(phashes[i])
            file_name_to_id.append((file_name, seq_global_id_bytes))
            id_to_file_name.append((seq_global_id_bytes, file_name))
            SEQUENTIAL_GLOBAL_ID += 1
